...
if(isset($EXECUTE) && $EXECUTE==1) {
    ...
    do {
        ...
        if(isset($data[0])){
            $RAG=filter_var($data[0],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }
        
        if(isset($data[1])){
            $DUE_DATE=filter_var($data[1],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }
        
        if(isset($data[2])){
            $EVENT=filter_var($data[2],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }
        ...
        if(isset($data[6])){
            $COVER_TYPE=filter_var($data[6],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }  
        if(isset($data[7])){
        ...
        if(isset($data[14])){
            $MTH_PREM=filter_var($data[14],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }         
        if(isset($data[15])){
            $MISSED_PREMS=filter_var($data[15],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }       
        if(isset($data[16])){
            $AMOUNT_DUE=filter_var($data[16],FILTER_SANITIZE_NUMBER_INT); 
        }     
        if(isset($data[17])){
            $COMM_UNEARNED=filter_var($data[17],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }         
        if(isset($data[18])){
            $COMM_CB=filter_var($data[18],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }  
        if(isset($data[19])){
            $SELLER_FCA_NUMBER=filter_var($data[19],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }  
        if(isset($data[20])){
            $SELLER_FIRM_NAME=filter_var($data[20],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }   
        if(isset($data[21])){
            $SELLER_NUMBER=filter_var($data[21],FILTER_SANITIZE_NUMBER_INT); 
        }      
        if(isset($data[22])){
            $SELLER_NAME=filter_var($data[22],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }           
        if(isset($data[23])){
            $BID=filter_var($data[23],FILTER_SANITIZE_NUMBER_INT); 
        }               
        if(isset($data[24])){
            $NID=filter_var($data[24],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }  
        if(isset($data[25])){
            $REPORT_RUN_DATE=filter_var($data[25],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }          
        
        $INSURER='LV';
        
        $NAME = $TITLE. ' ' . $FORENAME. ' ' . $SURNAME;
        $STATUS = "Outstanding premiums";
...
//INSERT NOTE INTO CLIENT TIMELINE    
    ...
    */
    ...
    if ($CHK_ADL_WARNINGS->rowCount() <= 0) { // INSERT THE REST
        ...
        $INSERT_EWS = $pdo->prepare('INSERT INTO adl_ews_lv
            SET 
                adl_ews_lv_id_fk=:LID,
                ...
                adl_ews_lv_policynumber=:POLICY, 
                adl_ews_lv_cover_type=:COVER_TYPE, 
                adl_ews_lv_title=:TITLE, 
                adl_ews_lv_forenames=:FORENAME, 
                adl_ews_lv_surname=:SURNAME, 
                ...
                adl_ews_lv_seller_fca_number=:FCA_NUMBER, 
                adl_ews_lv_seller_firm_name=:FIRM_NAME, 
                adl_ews_lv_seller_number=:SELLER_NUM, 
                adl_ews_lv_seller_name=:SELLER_NAME, 
                adl_ews_lv_seller_bid=:BID, 
                adl_ews_lv_seller_nid=:NID, 
                adl_ews_lv_report_run_date=:RUN_DATE');     
        $INSERT_EWS->bindParam(':LID',$LID, PDO::PARAM_INT);
        $INSERT_EWS->bindParam(':RAG',$RAG, PDO::PARAM_STR);
        $INSERT_EWS->bindParam(':DUE_DATE',$DUE_DATE, PDO::PARAM_STR);
        $INSERT_EWS->bindParam(':EVENT',$EVENT, PDO::PARAM_STR);
        ...
        $INSERT_EWS->bindParam(':COVER_TYPE',$COVER_TYPE, PDO::PARAM_STR);
        ...
        $INSERT_EWS->bindParam(':HOME',$HOME_NUM, PDO::PARAM_STR);
        $INSERT_EWS->bindParam(':MOB',$MOB_NUM, PDO::PARAM_STR);
        $INSERT_EWS->bindParam(':APE',$APE, PDO::PARAM_STR);
        $INSERT_EWS->bindParam(':MTH_PREM',$MTH_PREM, PDO::PARAM_STR);
        $INSERT_EWS->bindParam(':MISSED_PREM',$MISSED_PREMS, PDO::PARAM_STR);
        $INSERT_EWS->bindParam(':AMOUNT_DUE',$AMOUNT_DUE, PDO::PARAM_STR);
        $INSERT_EWS->bindParam(':UNEARNED_AMOUNT',$COMM_UNEARNED, PDO::PARAM_STR);
        ...
        $INSERT_EWS->bindParam(':SELLER_NUM',$SELLER_NUMBER, PDO::PARAM_INT);
