...
<body>
    ...
    ?>
        ...
        <ul class="nav nav-pills">
            ...
            <li><a data-toggle="pill" href="#NOMATCH">Unmatched Policies <span class="badge alert-warning">
                <?php $nomatchbadge = $pdo->query("select count(vitality_nomatch_id) AS badge from vitality_nomatch");
  ...
  <label class="col-md-4 control-label" for="query"></label>
    ...
    <div class="tab-content">
        ...
        <div class="panel-body">
                ...
                <form action=" " method="get">
                    ...
                    $COM_DATE_query = $pdo->prepare("SELECT DATE(vitality_insert_date) AS vitality_insert_date FROM vitality_financials group by DATE(vitality_insert_date) ORDER BY vitality_insert_date DESC");
                    ...
                    if ($COM_DATE_query->rowCount()>0) {
                        while ($row=$COM_DATE_query->fetch(PDO::FETCH_ASSOC)){ 
                                if(isset($row['vitality_insert_date'])) {  ?>
                                <option value="<?php echo $row['vitality_insert_date']; ?>"><?php echo $row['vitality_insert_date']; ?></option>
    ...
    SUM(CASE WHEN vitality_financials.vitality_comm < 0 THEN vitality_financials.vitality_comm ELSE 0 END) as totalloss,
...
$POL_ON_TM_QRY = $pdo->prepare("select 
    SUM(CASE WHEN vitality_financials.vitality_comm >= 0 THEN vitality_financials.vitality_comm ELSE 0 END) as PAID_TOTAL_PLUS,
    SUM(CASE WHEN vitality_financials.vitality_comm < 0 THEN vitality_financials.vitality_comm ELSE 0 END) as PAID_TOTAL_LOSS 
    FROM vitality_financials 
LEFT JOIN client_policy ON vitality_financials.vitality_policy=client_policy.policy_number 
WHERE DATE(vitality_financials.vitality_insert_date) = :commdate AND client_policy.policy_number IN(select client_policy.policy_number from client_policy WHERE DATE(client_policy.sale_date) between :datefrom AND :dateto)");
    ...
    SUM(CASE WHEN vitality_financials.vitality_comm >= 0 THEN vitality_financials.vitality_comm ELSE 0 END) as NOT_PAID_TOTAL_PLUS,
    SUM(CASE WHEN vitality_financials.vitality_comm < 0 THEN vitality_financials.vitality_comm ELSE 0 END) as NOT_PAID_TOTAL_LOSS        
FROM vitality_financials 
LEFT JOIN client_policy ON vitality_financials.vitality_policy=client_policy.policy_number 
WHERE DATE(vitality_financials.vitality_insert_date) = :commdate AND client_policy.policy_number IN(select client_policy.policy_number from client_policy WHERE DATE(client_policy.sale_date) NOT BETWEEN :datefrom AND :dateto)");
...
WHERE DATE(client_policy.sale_date) between :datefrom AND :dateto AND client_policy.insurer='Vitality' AND client_policy.policystatus IN ('Awaiting Policy Number','Live Awaiting Policy Number')");
        ...
        $ORIG_PENDING_SUM_FOR = number_format($ORIG_PENDING_SUM, 2); 
                    ...
                    $query = $pdo->prepare("SELECT SUM(CASE WHEN vitality_financials.vitality_comm<0 THEN vitality_financials.vitality_comm ELSE 0 END) as totalloss,
     SUM(CASE WHEN vitality_financials.vitality_comm>=0 THEN vitality_financials.vitality_comm ELSE 0 END) as totalgross
    FROM vitality_financials ");
...
$query = $pdo->prepare("select client_policy.id AS PID, client_policy.client_id AS CID, client_policy.policy_number, client_policy.commission, DATE(client_policy.sale_date) AS SALE_DATE, vitality_financials.vitality_client, vitality_financials.vitality_policy, vitality_financials.vitality_comm, DATE(vitality_financials.vitality_insert_date) AS COMM_DATE 
FROM vitality_financials 
LEFT JOIN client_policy ON vitality_financials.vitality_policy=client_policy.policy_number 
WHERE DATE(vitality_financials.vitality_insert_date) = :commdate ORDER by vitality_financials.vitality_comm DESC");
    $query->bindParam(':commdate', $commdate, PDO::PARAM_STR, 100);
$query->execute()or die(print_r($query->errorInfo(), true));
if ($query->rowCount()>0) {
...
while ($row=$query->fetch(PDO::FETCH_ASSOC)){
    ...
    echo "<td>".$row['vitality_client']."</td>";
...
?>
    ...
    $POLIN_SUM_QRY = $pdo->prepare("select sum(vitality_financials.vitality_comm) AS vitality_comm FROM vitality_financials 
LEFT JOIN client_policy ON vitality_financials.vitality_policy=client_policy.policy_number 
WHERE DATE(vitality_financials.vitality_insert_date) = :commdate AND client_policy.policy_number IN(select client_policy.policy_number from client_policy WHERE DATE(client_policy.sale_date) between :datefrom AND :dateto)");
    ...
    $POLIN_SUM_QRY->bindParam(':datefrom', $datefrom, PDO::PARAM_STR, 100);
        ...
        $ORIG_POLIN_SUM = $POLIN_SUM_QRY_RS['vitality_comm'];                                         
...
FROM vitality_financials 
LEFT JOIN client_policy ON vitality_financials.vitality_policy=client_policy.policy_number 
WHERE DATE(vitality_financials.vitality_insert_date) = :commdate AND client_policy.policy_number IN(select client_policy.policy_number from client_policy WHERE DATE(client_policy.sale_date) NOT BETWEEN :datefrom AND :dateto) AND vitality_financials.Payment_Amount >='0'");
...
$policy = $row['vitality_policy'];
$PAY_AMOUNT = number_format($row['vitality_comm'], 2);
...
$query = $pdo->prepare("select vitality_nomatch_insert_date, vitality_nomatch_id, vitality_nomatch_policy, vitality_nomatch_comm from vitality_nomatch");
?>
                <table class="table table-hover">
                    <thead>
                        <tr>
...
$policy = $row['vitality_nomatch_policy'];
$AMOUNT = $row['vitality_nomatch_comm'];
$iddd = $row['vitality_nomatch_id'];

    echo '<tr>'; 
    echo"<td>".$row['vitality_nomatch_insert_date']."</td>";
    echo "<td>$policy</td>"; 
       if (intval($row['vitality_nomatch_comm'])>0) {
       echo "<td><span class=\"label label-success\">".$row['vitality_nomatch_comm']."</span></td>"; }
       else if (intval($row["vitality_nomatch_comm"])<0) {
           echo "<td><span class=\"label label-danger\">".$row['vitality_nomatch_comm']."</span></td>"; }
           else {
               echo "<td>".$row['vitality_nomatch_comm']."</td>"; }
               
               if(isset($datefrom)) {
                                  echo "<td><a href='../php/Financial_Recheck.php?EXECUTE=1&INSURER=Vitality&RECHECK=y&finpolicynumber=$policy&AMOUNT=$AMOUNT&iddd=$iddd&datefrom=$datefrom&dateto=$dateto&commdate=$commdate' class='btn btn-success btn-sm'><i class='fa fa-check-circle-o'></i></a></td>";

               }
               
               else {
                                  echo "<td><a href='../php/Financial_Recheck.php?EXECUTE=1&INSURER=Vitality&RECHECK=y&finpolicynumber=$policy&AMOUNT=$AMOUNT&iddd=$iddd' class='btn btn-success btn-sm'><i class='fa fa-check-circle-o'></i></a></td>";
