...
?>
    ...
    <div class="container">
        <div class="notice notice-default" role="alert"><strong><center><span class="label label-warning"></span> Vitality Audits</center></strong></div>
        
        <?php
        if(isset($return)) {
            if($return=='Edit'){
                echo "<div class=\"notice notice-success\" role=\"alert\"><strong><i class=\"fa fa-edit fa-lg\"></i> Success:</strong> Vitality Audit Updated!</div>";
                
            }
            if($return=='Add') {
                echo "<div class=\"notice notice-success\" role=\"alert\"><strong><i class=\"fa fa-check-circle-o\"></i> Success:</strong> Vitality Audit Added!</div>";
...
<br>
    ...
    <?php 
    $CHK_SAVED = $pdo->prepare("select count(vitality_audit_id) AS vitality_audit_id from vitality_audit where vitality_audit_auditor ='SAVED' and vitality_audit_grade =:hello");
    $CHK_SAVED->bindParam(':hello', $hello_name, PDO::PARAM_STR, 12);
    $CHK_SAVED->execute();
    if ($CHK_SAVED->rowCount()>0) {
        while ($result=$CHK_SAVED->fetch(PDO::FETCH_ASSOC)){
            $savedcount = $result['vitality_audit_id'];
            if ($savedcount >=1){
                ...
                $query = $pdo->prepare("SELECT
                                            ...
                                            vitality_audit_id, 
                                            vitality_audit_added_date, 
                                            vitality_audit_closer, 
                                            vitality_audit_auditor, 
                                            vitality_audit_grade, 
                                            vitality_audit_updated_by, 
                                            vitality_audit_updated_date 
                                        FROM 
                                            vitality_audit 
                                        WHERE 
                                        ...
                                        OR 
                                            vitality_audit_updated_date between DATE_ADD(CURDATE(), INTERVAL 1-DAYOFWEEK(CURDATE()) DAY) 
                                        AND 
                                            DATE_ADD(CURDATE(), INTERVAL 7-DAYOFWEEK(CURDATE()) DAY) 
                                        AND 
                                            vitality_audit_updated_by =:hello 
                                        ORDER BY 
                                            vitality_audit_added_date DESC");
	...
	<th colspan='5'>Options</th>
                    ...
                    while ($result=$query->fetch(PDO::FETCH_ASSOC)){
                        $i++;
                        $AUDIT_ID=$result['vitality_audit_id'];
                        
                        switch( $result['vitality_audit_grade'] ) {
                            case("Red"):
                                $class = 'Red';
                                break;
                            case("Green"):
                            ...
                            default:
                                ...
                                echo '<tr class='.$class.'>';
                                echo "<td>".$result['vitality_audit_id']."</td>";
                                echo "<td>".$result['vitality_audit_plan_number']."</td>";
                                echo "<td>".$result['vitality_audit_added_date']."</td>";
   ...
   echo "<td><a href='View.php?EXECUTE=VIEW&AUDITID=$AUDIT_ID' class='btn btn-info btn-xs'><span class='glyphicon glyphicon-eye-open'></span></a></td>";
    ...
    echo "<br><div class=\"notice notice-warning\" role=\"alert\"><strong>Info!</strong> No Vitality Audits found</div>";
}
...
<?php
$query = $pdo->prepare("SELECT vitality_audit_plan_number, vitality_audit_id, vitality_audit_added_date, vitality_audit_closer, vitality_audit_auditor, vitality_audit_grade from vitality_audit where vitality_audit_auditor = :hello and vitality_audit_grade = 'Saved' ORDER BY vitality_audit_added_date DESC");
$query->bindParam(':hello', $hello_name, PDO::PARAM_STR, 12);
echo "<table align=\"center\" class=\"table\">";
