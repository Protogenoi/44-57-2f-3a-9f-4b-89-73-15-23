...
$insert = $pdo->prepare("INSERT INTO Set_Client_Tasks set Assigned=:assign, Task=:task");
        ...
        if ($insert->rowCount()>=1) {
            ...
            if(isset($fferror)) {
                if($fferror=='0') {
                    header('Location: ../Admindash.php?TaskAssigned=y&AssignTasks=y&TaskAssignedTo='.$taskuser.'&TASKUPDATED='.$taskname); die;
                    }
                    }
        ...
        if(isset($fferror)) {
    if($fferror=='0') {
    header('Location: ../Admindash.php?TaskAssigned=failed&AssignTasks=y'); die;
    }
        }
