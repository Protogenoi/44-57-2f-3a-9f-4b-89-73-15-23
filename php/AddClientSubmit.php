...
if(isset($fferror)) {
    ...
    if($custype=='Pension') {
        ...
        if(isset($add)) {
            ...
            $database->query("INSERT INTO pension_clients SET residence=:residence, marital=:marital, ni_num=:ninum, ni_num2=:ninum2, title=:title, first_name=:first, initials=:initials, last_name=:last, dob=:dob, title2=:titlehold, first_name2=:firsthold, initials2=:initialshold, last_name2=:lasthold, dob2=:dobhold, address1=:add1, address2=:add2, address3=:add3, town=:town, post_code=:post, number1=:num1, number2=:num2, number3=:num3, email=:email, added_by=:hello");
            $database->bind(':residence',$residence);
            $database->bind(':marital',$marital);
            $database->bind(':title',$title);
            $database->bind(':first',$first_name);
            $database->bind(':initials',$initials);
            $database->bind(':last',$last_name);
            ...
            $database->bind(':post',$post_code);
            $database->bind(':num1',$num1);
            $database->bind(':num2',$num2);
            $database->bind(':num3',$num3);
            $database->bind(':email',$email);
            $database->bind(':hello',$hello_name);
            $database->bind(':titlehold',$title2);
            $database->bind(':firsthold',$first_name2);
            $database->bind(':initialshold',$initials2);
            $database->bind(':lasthold',$last_name2);
            $database->bind(':dobhold',$dob2);
            $database->bind(':ninum',$ni_num);
            $database->bind(':ninum2',$ni_num2);
            $database->execute(); 
            $lastid =  $database->lastInsertId();

            
            $notedata= "Client Added";
            $custypenamedata= $title ." ". $first_name ." ". $last_name;
            ...
            header('Location: /Pensions/AddNewPension.php?Pension=y&search='.$lastid); die;
