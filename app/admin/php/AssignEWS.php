...
$dupe = $pdo->prepare("SELECT status FROM assign_ews_status WHERE status=:STATUS");
        ...
        if ($INSERT->rowCount()>=1) {
            ...
            if(isset($fferror)) {
                if($fferror=='0') {
                    header('Location: ../Admindash.php?EWSassigned=y&EWSSelect=y&EWSassignedTo='.$EWS_USER.'&TASKUPDATED='.$EWS_STATUS); die;
                    }
                    }
        ...
        if(isset($fferror)) {
    if($fferror=='0') {
    header('Location: ../Admindash.php?EWSassigned=failed&EWSSelect=y'); die;
    }
        }
