...
if(isset($EXECUTE) && $EXECUTE==1) {
    ...
    do {
        ...
        if(isset($data[19])){
            $NOTES=filter_var($data[19],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        } 
        if(isset($data[20])){
            $COLOUR=filter_var($data[20],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }  
        if(isset($data[21])){
            $NEW_STATUS=filter_var($data[21],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }          
...
FROM
    adl_ews
        JOIN
    adl_ews_royal_london ON adl_ews_royal_london_id_fk = adl_ews_id
WHERE
    adl_ews_client_name =:NAME
        AND adl_ews_orig_status = :STATUS
        AND adl_ews_ref =:REF
        AND adl_ews_insurer =:INSURER
        AND adl_ews_royal_london_arrears_start_date =:ARREARS");
                    ...
                    $CHK_ADL_WARNINGS->bindParam(':ARREARS',$ARREARS_START_DATE, PDO::PARAM_STR);
...
//UPDATE EWS UPDATED DATE
                ...
                $UPDATE_EWS = $pdo->prepare('
                                            ...
                                            SET 
                                                ...
                                                adl_ews_updated_by="ADL",
                                                adl_ews_notes=:NOTES,
                                                adl_ews_colour=:COLOUR,
                                                adl_ews_status=:NEW_STATUS
                ...
                $UPDATE_EWS->bindParam(':COLOUR',$COLOUR, PDO::PARAM_STR); 
                $UPDATE_EWS->bindParam(':NOTES',$NOTES, PDO::PARAM_STR); 
                $UPDATE_EWS->bindParam(':NEW_STATUS',$NEW_STATUS, PDO::PARAM_STR); 
                ...
                $INSERT_MASTER = $pdo->prepare('
                                            ...
                                            SET 
                                                ...
                                                adl_ews_master_updated_by="ADL",
                                                adl_ews_master_status=:NEW_STATUS
                                            ');     
                ...
                $INSERT_MASTER->bindParam(':NOTES',$NOTES, PDO::PARAM_STR); 
                $INSERT_MASTER->bindParam(':NEW_STATUS',$NEW_STATUS, PDO::PARAM_STR);                 
...
//MATCH POLICY TO ADL TO GET CLIENT ID        
    ...
    $note="Royal London EWS Master Uploaded";
    $ref= "$POL_NUMBER ($PID)";
    $messageEWS="$ORIG_WARNING updated to $NEW_STATUS (EWS MASTER)";
...
//INSERT NOTE INTO CLIENT TIMELINE    
    ...
    $note="$INSURER EWS Master Uploaded";
    $ref= "$POL_NUMBER ($PID)";
    $MESSAGE = "$POLICY is now $NEW_STATUS (EWS MASTER)";
