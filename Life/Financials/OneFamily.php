...
<body>
    ...
    ?>
        ...
        <ul class="nav nav-pills">
            ...
            <li><a data-toggle="pill" href="#NOMATCH">Unmatched Policies <span class="badge alert-warning">
                <?php $nomatchbadge = $pdo->query("select count(wol_nomatch_id) AS badge from wol_nomatch");
  ...
  <label class="col-md-4 control-label" for="query"></label>
    ...
    <div class="tab-content">
        ...
        <div class="panel-body">
                ...
                <form action=" " method="get">
                    ...
                    $COM_DATE_query = $pdo->prepare("SELECT DATE(wol_insert_date) AS wol_insert_date FROM wol_financials group by DATE(wol_insert_date) ORDER BY wol_insert_date DESC");
                    ...
                    if ($COM_DATE_query->rowCount()>0) {
                        while ($row=$COM_DATE_query->fetch(PDO::FETCH_ASSOC)){ 
                                if(isset($row['wol_insert_date'])) {  ?>
                                <option value="<?php echo $row['wol_insert_date']; ?>"><?php echo $row['wol_insert_date']; ?></option>
    ...
    SUM(CASE WHEN wol_financials.wol_comm  < 0 THEN wol_financials.wol_comm  ELSE 0 END) as totalloss,
    SUM(CASE WHEN wol_financials.wol_comm  >= 0 THEN wol_financials.wol_comm  ELSE 0 END) as totalgross
    FROM wol_financials WHERE DATE(wol_insert_date)=:commdate");
    $query->bindParam(':commdate', $commdate, PDO::PARAM_STR, 100);

    $MISSING_SUM_QRY = $pdo->prepare("select sum(client_policy.commission) AS commission FROM client_policy LEFT JOIN wol_financials ON wol_financials.wol_policy=client_policy.policy_number WHERE DATE(client_policy.sale_date) between :datefrom AND :dateto AND client_policy.policy_number NOT IN(select wol_financials.wol_policy from wol_financials) AND client_policy.insurer='One Family' AND client_policy.policystatus NOT like '%CANCELLED%' AND client_policy.policystatus NOT IN ('Awaiting Policy Number','Clawback','SUBMITTED-NOT-LIVE','DECLINED') AND client_policy.policy_number NOT like '%DU%' AND client_policy.policy_number NOT like '%tbc%'");
        ...
        $EXPECTED_SUM_QRY = $pdo->prepare("select SUM(commission) AS commission FROM client_policy WHERE DATE(sale_date) between :datefrom AND :dateto AND insurer='One Family' AND client_policy.policystatus NOT like '%CANCELLED%' AND client_policy.policystatus NOT IN ('Clawback','SUBMITTED-NOT-LIVE','DECLINED','On hold') AND client_policy.policy_number NOT like '%DU%'");
...
$POL_ON_TM_QRY = $pdo->prepare("select 
    SUM(CASE WHEN wol_financials.wol_comm  >= 0 THEN wol_financials.wol_comm  ELSE 0 END) as PAID_TOTAL_PLUS,
    SUM(CASE WHEN wol_financials.wol_comm  < 0 THEN wol_financials.wol_comm  ELSE 0 END) as PAID_TOTAL_LOSS 
    FROM wol_financials 
LEFT JOIN client_policy ON wol_financials.wol_policy=client_policy.policy_number 
WHERE DATE(wol_financials.wol_insert_date) = :commdate AND client_policy.policy_number IN(select client_policy.policy_number from client_policy WHERE DATE(client_policy.sale_date) between :datefrom AND :dateto)");
    ...
    SUM(CASE WHEN wol_financials.wol_comm  >= 0 THEN wol_financials.wol_comm  ELSE 0 END) as NOT_PAID_TOTAL_PLUS,
    SUM(CASE WHEN wol_financials.wol_comm  < 0 THEN wol_financials.wol_comm  ELSE 0 END) as NOT_PAID_TOTAL_LOSS        
FROM wol_financials 
LEFT JOIN client_policy ON wol_financials.wol_policy=client_policy.policy_number 
WHERE DATE(wol_financials.wol_insert_date) = :commdate AND client_policy.policy_number IN(select client_policy.policy_number from client_policy WHERE DATE(client_policy.sale_date) NOT BETWEEN :datefrom AND :dateto)");
...
LEFT JOIN wol_financials ON wol_financials.wol_policy=client_policy.policy_number 
WHERE DATE(client_policy.sale_date) between :datefrom AND :dateto AND client_policy.insurer='One Family' AND client_policy.policystatus IN ('Awaiting Policy Number','Live Awaiting Policy Number')");
        ...
        $PENDING_SUM_QRY = $pdo->prepare("select SUM(commission) AS commission FROM client_policy WHERE DATE(sale_date) BETWEEN '2017-01-01' AND :dateto AND policy_number NOT IN(select wol_policy from wol_financials) AND insurer='One Family' AND policystatus NOT like '%CANCELLED%' AND policystatus NOT IN ('Live Awaiting Policy Number','Awaiting Policy Number','Clawback','SUBMITTED-NOT-LIVE','DECLINED') AND policy_number NOT like '%DU%'");
        ...
        $ORIG_PENDING_SUM_FOR = number_format($ORIG_PENDING_SUM, 2); 
                    ...
                    $query = $pdo->prepare("SELECT SUM(CASE WHEN wol_financials.wol_comm <0 THEN wol_financials.wol_comm  ELSE 0 END) as totalloss,
     SUM(CASE WHEN wol_financials.wol_comm >=0 THEN wol_financials.wol_comm  ELSE 0 END) as totalgross
    FROM wol_financials ");
...
FROM client_policy
LEFT JOIN wol_financials ON wol_financials.wol_policy=client_policy.policy_number 
WHERE client_policy.policy_number NOT IN(select wol_financials.wol_policy from wol_financials) AND client_policy.insurer='One Family' AND client_policy.policystatus IN ('Awaiting Policy Number','Live Awaiting Policy Number')");
        ...
        $TBC_SUM = number_format($TBC_SUM_UNFORMATTED, 2);
         ...
         $PENDING_SUM_QRY = $pdo->prepare("select SUM(commission) AS commission FROM client_policy WHERE DATE(sale_date) BETWEEN '2017-01-01' AND CURDATE() AND policy_number NOT IN(select wol_policy from wol_financials) AND insurer='One Family' AND policystatus NOT like '%CANCELLED%' AND policystatus NOT IN ('Live Awaiting Policy Number','Awaiting Policy Number','Clawback','SUBMITTED-NOT-LIVE','DECLINED') AND policy_number NOT like '%DU%'"); 
        $PENDING_SUM_QRY->execute()or die(print_r($PENDING_SUM_QRY->errorInfo(), true));
        $PENDING_SUM_QRY_RS=$PENDING_SUM_QRY->fetch(PDO::FETCH_ASSOC);
        $ORIG_PENDING_SUM = $PENDING_SUM_QRY_RS['commission'];                    
...
$query = $pdo->prepare("select client_policy.id AS PID, client_policy.client_id AS CID, client_policy.policy_number, client_policy.commission, DATE(client_policy.sale_date) AS SALE_DATE, wol_financials.wol_client, wol_financials.wol_policy, wol_financials.wol_comm , DATE(wol_financials.wol_insert_date) AS COMM_DATE 
FROM wol_financials 
LEFT JOIN client_policy ON wol_financials.wol_policy=client_policy.policy_number 
WHERE DATE(wol_financials.wol_insert_date) = :commdate ORDER by wol_financials.wol_comm  DESC");
    $query->bindParam(':commdate', $commdate, PDO::PARAM_STR, 100);
$query->execute()or die(print_r($query->errorInfo(), true));
if ($query->rowCount()>0) {
...
FROM client_policy
WHERE insurer='One Family' AND DATE(sale_date) between :datefrom AND :dateto AND client_policy.policy_number NOT like '%tbc%' AND client_policy.policy_number NOT like '%DU%'");
...
FROM client_policy
WHERE DATE(sale_date) BETWEEN '2017-01-01' AND :dateto AND policy_number NOT IN(select wol_policy from wol_financials) AND insurer='One Family' AND policystatus NOT like '%CANCELLED%' AND policystatus NOT IN ('Live Awaiting Policy Number','Awaiting Policy Number','Clawback','SUBMITTED-NOT-LIVE','DECLINED','On hold') AND policy_number NOT like '%DU%' ORDER BY commission DESC");
    $query->bindParam(':dateto', $dateto, PDO::PARAM_STR, 100);
$query->execute()or die(print_r($query->errorInfo(), true));
if ($query->rowCount()>0) {
    $count = $query->rowCount();
...
$query = $pdo->prepare("select DATE(sale_date) AS SALE_DATE, policystatus, client_name, id AS PID, client_id AS CID, policy_number, commission 
FROM client_policy
WHERE DATE(sale_date) BETWEEN '2017-01-01' AND CURDATE() AND policy_number NOT IN(select wol_policy from wol_financials) AND insurer='One Family' AND policystatus NOT like '%CANCELLED%' AND policystatus NOT IN ('Live Awaiting Policy Number','Awaiting Policy Number','Clawback','SUBMITTED-NOT-LIVE','DECLINED','On hold') AND policy_number NOT like '%DU%' ORDER BY commission DESC");
$query->execute()or die(print_r($query->errorInfo(), true));
if ($query->rowCount()>0) {
    $count = $query->rowCount();
    ?>
...
$query = $pdo->prepare("select DATE(client_policy.sale_date) AS SALE_DATE, client_policy.policystatus, client_policy.client_name, client_policy.id AS PID, client_policy.client_id AS CID, client_policy.policy_number, client_policy.commission, DATE(client_policy.sale_date) AS SALE_DATE, wol_financials.wol_policy, wol_financials.wol_comm , DATE(wol_financials.wol_insert_date) AS COMM_DATE 
FROM client_policy
LEFT JOIN wol_financials ON wol_financials.wol_policy=client_policy.policy_number 
WHERE DATE(client_policy.sale_date) between :datefrom AND :dateto AND client_policy.policy_number NOT IN(select wol_financials.wol_policy from wol_financials) AND client_policy.policy_number NOT IN(select wol_financials.wol_policy from wol_financials) AND client_policy.insurer='One Family' AND client_policy.policystatus NOT like '%CANCELLED%' AND client_policy.policystatus NOT IN ('Live Awaiting Policy Number','Awaiting Policy Number','Clawback','SUBMITTED-NOT-LIVE','DECLINED') AND client_policy.policy_number NOT like '%DU%' AND client_policy.policy_number NOT like '%tbc%' ");
    $query->bindParam(':datefrom', $datefrom, PDO::PARAM_STR, 100);
    $query->bindParam(':dateto', $dateto, PDO::PARAM_STR, 100);
$query->execute()or die(print_r($query->errorInfo(), true));
if ($query->rowCount()>0) {
...
$query = $pdo->prepare("select DATE(client_policy.sale_date) AS sale_date, client_policy.policystatus, client_policy.client_name, client_policy.id AS PID, client_policy.client_id AS CID, client_policy.policy_number, client_policy.commission, wol_financials.wol_policy, wol_financials.wol_comm , DATE(wol_financials.wol_insert_date) AS COMM_DATE 
FROM client_policy
LEFT JOIN wol_financials ON wol_financials.wol_policy=client_policy.policy_number 
WHERE DATE(client_policy.sale_date) between :datefrom AND :dateto AND client_policy.insurer='One Family' AND client_policy.policystatus IN ('Awaiting Policy Number','Live Awaiting Policy Number') ORDER BY DATE(client_policy.sale_date)");
    $query->bindParam(':datefrom', $datefrom, PDO::PARAM_STR, 100);
    $query->bindParam(':dateto', $dateto, PDO::PARAM_STR, 100);
$query->execute()or die(print_r($query->errorInfo(), true));
if ($query->rowCount()>0) {
...
FROM client_policy
LEFT JOIN wol_financials ON wol_financials.wol_policy=client_policy.policy_number 
WHERE client_policy.policy_number NOT IN(select wol_financials.wol_policy from wol_financials) AND client_policy.insurer='One Family' AND client_policy.policystatus IN ('Awaiting Policy Number','Live Awaiting Policy Number') OR client_policy.policy_number like '%tbc%' AND client_policy.insurer='One Family' ORDER BY client_policy.commission DESC");
$query->execute()or die(print_r($query->errorInfo(), true));
if ($query->rowCount()>0) {
    $count = $query->rowCount();
    ?>
...
?>
    ...
    $POLIN_SUM_QRY = $pdo->prepare("select sum(wol_financials.wol_comm ) AS wol_comm  FROM wol_financials 
LEFT JOIN client_policy ON wol_financials.wol_policy=client_policy.policy_number 
WHERE DATE(wol_financials.wol_insert_date) = :commdate AND client_policy.policy_number IN(select client_policy.policy_number from client_policy WHERE DATE(client_policy.sale_date) between :datefrom AND :dateto)");
    ...
    $POLIN_SUM_QRY->bindParam(':datefrom', $datefrom, PDO::PARAM_STR, 100);
        ...
        $ORIG_POLIN_SUM = $POLIN_SUM_QRY_RS['wol_comm'];                                         
    
$query = $pdo->prepare("select client_policy.client_name, client_policy.id AS PID, client_policy.client_id AS CID, client_policy.policy_number, client_policy.commission, DATE(client_policy.sale_date) AS SALE_DATE, wol_financials.wol_policy, wol_financials.wol_comm , DATE(wol_financials.wol_insert_date) AS COMM_DATE 
FROM wol_financials 
LEFT JOIN client_policy ON wol_financials.wol_policy=client_policy.policy_number 
WHERE DATE(wol_financials.wol_insert_date) = :commdate AND client_policy.policy_number IN(select client_policy.policy_number from client_policy WHERE DATE(client_policy.sale_date) between :datefrom AND :dateto)");
    $query->bindParam(':commdate', $commdate, PDO::PARAM_STR, 100);
...
FROM wol_financials 
LEFT JOIN client_policy ON wol_financials.wol_policy=client_policy.policy_number 
WHERE DATE(wol_financials.wol_insert_date) = :commdate AND client_policy.policy_number IN(select client_policy.policy_number from client_policy WHERE DATE(client_policy.sale_date) NOT BETWEEN :datefrom AND :dateto) AND wol_financials.Payment_Amount >='0'");
...
<div class="container">
    ...
    $COMMIN_SUM_QRY = $pdo->prepare("select sum(wol_financials.wol_comm ) AS wol_comm  from wol_financials LEFT JOIN client_policy on wol_financials.wol_policy=client_policy.policy_number where wol_financials.wol_comm  >= 0 AND DATE(wol_financials.wol_insert_date) =:commdate AND client_policy.insurer ='One Family'");
    $COMMIN_SUM_QRY->bindParam(':commdate', $commdate, PDO::PARAM_STR, 100);
        ...
        $ORIG_COMMIN_SUM = $COMMIN_SUM_QRY_RS['wol_comm'];                            
$COMMIN_SUM_FORMATTED = number_format($ORIG_COMMIN_SUM, 2);

$query = $pdo->prepare("select wol_financials.wol_comm , client_policy.CommissionType, DATE(client_policy.sale_date) AS sale_date, client_policy.policy_number, wol_financials.wol_policy, client_policy.client_name, client_policy.client_id from wol_financials LEFT JOIN client_policy on wol_financials.wol_policy=client_policy.policy_number where wol_financials.wol_comm  >= 0 AND DATE(wol_financials.wol_insert_date) =:commdate AND client_policy.insurer ='One Family'");
    $query->bindParam(':commdate', $commdate, PDO::PARAM_STR, 100);
$query->execute()or die(print_r($query->errorInfo(), true));
if ($query->rowCount()>0) {
    $count = $query->rowCount();
...
$policy = $row['wol_policy'];
$PAY_AMOUNT = number_format($row['wol_comm'], 2);
...
<div class="container">
                ...
                if(isset($datefrom)){
                       ...
                       $COMMOUT_SUM_QRY = $pdo->prepare("select sum(wol_financials.wol_comm ) AS wol_comm  from wol_financials LEFT JOIN client_policy on wol_financials.wol_policy=client_policy.policy_number where wol_financials.wol_comm  < 0 AND DATE(wol_financials.wol_insert_date) =:commdate AND client_policy.insurer ='One Family'");
    $COMMOUT_SUM_QRY->bindParam(':commdate', $commdate, PDO::PARAM_STR, 100);
        ...
        $ORIG_COMMOUT_SUM = $COMMOUT_SUM_QRY_RS['wol_comm'];                            
...
$query = $pdo->prepare("select wol_nomatch_insert_date, wol_nomatch_id, wol_nomatch_policy, wol_nomatch_comm from wol_nomatch");
?>
                <table class="table table-hover">
                    <thead>
                        <tr>
...
$policy = $row['wol_nomatch_policy'];
$AMOUNT = $row['wol_nomatch_comm'];
$iddd = $row['wol_nomatch_id'];

    echo '<tr>'; 
    echo"<td>".$row['wol_nomatch_insert_date']."</td>";
    echo "<td>$policy</td>"; 
       if (intval($row['wol_nomatch_comm'])>0) {
       echo "<td><span class=\"label label-success\">".$row['wol_nomatch_comm']."</span></td>"; }
       else if (intval($row["wol_nomatch_comm"])<0) {
           echo "<td><span class=\"label label-danger\">".$row['wol_nomatch_comm']."</span></td>"; }
           else {
               echo "<td>".$row['wol_nomatch_comm']."</td>"; }
               
               if(isset($datefrom)) {
                                  echo "<td><a href='../php/Financial_Recheck.php?EXECUTE=1&INSURER=OneFamily&RECHECK=y&finpolicynumber=$policy&AMOUNT=$AMOUNT&iddd=$iddd&datefrom=$datefrom&dateto=$dateto&commdate=$commdate' class='btn btn-success btn-sm'><i class='fa fa-check-circle-o'></i></a></td>";

               }
               
               else {
                                  echo "<td><a href='../php/Financial_Recheck.php?EXECUTE=1&INSURER=OneFamily&RECHECK=y&finpolicynumber=$policy&AMOUNT=$AMOUNT&iddd=$iddd' class='btn btn-success btn-sm'><i class='fa fa-check-circle-o'></i></a></td>";
