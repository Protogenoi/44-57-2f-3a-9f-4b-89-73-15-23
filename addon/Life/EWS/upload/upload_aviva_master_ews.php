...
if(isset($EXECUTE) && $EXECUTE==1) {
    ...
    do {
        ...
        if(isset($data[5])){
            $NOTES=filter_var($data[5],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        } 
        if(isset($data[6])){
            $COLOUR=filter_var($data[6],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }      
        if(isset($data[7])){
            $NEW_STATUS=filter_var($data[7],FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
        }         
...
//MATCH POLICY TO ADL TO GET CLIENT ID        
    ...
    $note="Aviva EWS Master Uploaded";
    $ref= "$POL_NUMBER ($PID)";
...
//INSERT NOTE INTO CLIENT TIMELINE    
    ...
    $ref= "$POL_NUMBER ($PID)";
    $MESSAGE = "$POLICY is now $NEW_STATUS";
