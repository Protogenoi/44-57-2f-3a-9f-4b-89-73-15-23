...
if(isset($EXECUTE) && $EXECUTE==10) {
    ...
    if($INSURER=='LV') {
        ...
        $i=0;
            ...
            $POL_NUM=$row['lv_financial_nomatch_policy_number'];
            $FID=$row['lv_financial_nomatch_id'];
            $AMOUNT=$row['lv_financial_nomatch_indemnity'];
    ...
    if($INSURER=='Vitality') {
     ...
     if($INSURER=='Aviva') {
        ...
        $i=0;
            ...
            $Icheck = $pdo->prepare("SELECT aviva_financial_nomatch_id, aviva_financial_nomatch_policy, aviva_financial_nomatch_amount FROM aviva_financial_nomatch");
            $Icheck->execute();
            if ($Icheck->rowCount() >= 1) {  
            while ($row=$Icheck->fetch(PDO::FETCH_ASSOC)){ 
            ...
            $AMOUNT=$row['aviva_financial_nomatch_amount'];
                ...
                if ($SELECT_Q->rowCount() >= 1) {  
                    ...
                    $update->execute();
                       ...
                       $delete = $pdo->prepare("DELETE FROM aviva_financial_nomatch WHERE aviva_financial_nomatch_policy=:pol AND aviva_financial_nomatch_id=:ID LIMIT 1");
                       $delete->bindParam(':pol', $policynumber, PDO::PARAM_STR, 250);
                       $delete->bindParam(':ID', $FID, PDO::PARAM_INT);
                       $delete->execute();  
