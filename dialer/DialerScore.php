...
<body class="backcolour">
    <div class="container">
        ...
        if(isset($top_hangup_result['hangup_cause'])) {
        ...
        if(isset($count_hangup_result['hangup_cause'])) {
        ...
        if(isset($hangup_percent)) {
            if($hangup_percent >= 49 && $hangup_percent <= 59) {
                $HANG_UP_MSG="Moderate";
                    ...
                    $CLOSER_query = $TRB_DB_PDO->prepare("SELECT vicidial_users.full_name, vicidial_live_agents.uniqueid, vicidial_live_agents.lead_id, vicidial_live_agents.status, vicidial_live_agents.pause_code, vicidial_auto_calls.phone_number, vicidial_users.full_name, vicidial_agent_log.campaign_id, TIMEDIFF(current_TIMESTAMP, vicidial_live_agents.last_state_change) as Time from vicidial_agent_log JOIN vicidial_users ON vicidial_users.user = vicidial_agent_log.user JOIN vicidial_live_inbound_agents on vicidial_users.full_name = vicidial_live_inbound_agents.group_id JOIN vicidial_live_agents ON vicidial_agent_log.user = vicidial_live_agents.user LEFT JOIN vicidial_list on vicidial_live_agents.lead_id =vicidial_list.lead_id LEFT JOIN vicidial_auto_calls on vicidial_live_agents.lead_id = vicidial_auto_calls.lead_id LEFT JOIN vicidial_lists on vicidial_list.list_id = vicidial_lists.list_id WHERE vicidial_agent_log.event_time >= CURRENT_DATE() AND vicidial_agent_log.campaign_id ='15' GROUP BY vicidial_agent_log.user ORDER BY vicidial_live_agents.campaign_id ASC, vicidial_live_agents.status ASC, last_state_change LIMIT 10");
                                    ...
                                    switch( $result['status'] ) {
                                        case("READY"):
                                            ...
                                            case("INCALL"):
                                                ...
                                                $PAUSEquery = $TRB_DB_PDO->prepare("SELECT count(live_agent_id) AS LIVE_AGENT_ID FROM vicidial_live_agents WHERE campaign_id ='10' AND status ='PAUSED' or status = 'DISPO'");
                                                $PAUSEquery->execute();
                                                $PAUSEresult=$PAUSEquery->fetch(PDO::FETCH_ASSOC);
                                                $PAUSEDCOUNT=$PAUSEresult['LIVE_AGENT_ID'];
                                                
                                                $PAUSED_AGENTS_query = $TRB_DB_PDO->prepare("SELECT vicidial_live_agents.comments, vicidial_live_agents.lead_id, vicidial_auto_calls.phone_number, vicidial_live_agents.status, vicidial_users.full_name, vicidial_live_agents.pause_code, vicidial_live_agents.uniqueid, TIMEDIFF(current_TIMESTAMP, vicidial_live_agents.last_state_change) as Time FROM vicidial_live_agents JOIN vicidial_users on vicidial_live_agents.user = vicidial_users.user LEFT JOIN vicidial_auto_calls on vicidial_live_agents.lead_id = vicidial_auto_calls.lead_id WHERE vicidial_live_agents.campaign_id ='10' AND vicidial_live_agents.status ='PAUSED' or vicidial_live_agents.status = 'DISPO' order by vicidial_live_agents.status, last_state_change");
                        ...
                        if(isset($result['pause_code'])) {
                            ...
                            if ($ii % 5 == 0) { 
                                ...
                                if(isset($CHECK_TOP_PAUSES)) {
                                    ...
                                    $PAUSE_STATUS_query = $TRB_DB_PDO->prepare("SELECT vicidial_users.full_name, sum(vicidial_agent_log.pause_sec)+SUM(vicidial_agent_log.dispo_sec)+SUM(vicidial_agent_log.dead_sec) AS Pause_Sec_TIME from vicidial_agent_log JOIN vicidial_users on vicidial_agent_log.user = vicidial_users.user where campaign_id IN ('10') and vicidial_agent_log.event_time >= :DATE_SUB_1 and vicidial_agent_log.event_time <= :DATE_SUB_2 GROUP BY vicidial_agent_log.user ORDER BY Pause_Sec_TIME DESC LIMIT 5");
