...
if(isset($changereason)) {
    ...
    if($changereason=='Client Cancelled' && $active=='0') {
   
             
        $addquery = $pdo->prepare("UPDATE pension_clients SET active=:active, residence=:residence, marital=:marital, ni_num=:ninum, ni_num2=:ninum2, title=:title, first_name=:first, initials=:initials, last_name=:last, dob=:dob, title2=:titlehold, first_name2=:firsthold, initials2=:initialshold, last_name2=:lasthold, dob2=:dobhold, address1=:add1, address2=:add2, address3=:add3, town=:town, post_code=:post, number1=:num1, number2=:num2, number3=:num3, email=:email, updated_by=:hello WHERE client_id=:clientid");
        $addquery->bindParam(':active',$active, PDO::PARAM_STR, 255);
        $addquery->bindParam(':marital',$marital, PDO::PARAM_STR, 255);
        $addquery->bindParam(':residence',$residence, PDO::PARAM_STR, 255);
        $addquery->bindParam(':clientid',$searchid, PDO::PARAM_INT);
        ...
        $query->bindParam(':clientidholder',$searchid, PDO::PARAM_INT);
        ...
        $cancelSTAGES = $pdo->prepare("UPDATE pension_stages set active_stage='N' WHERE client_id =:id");
        $cancelSTAGES->bindParam(':id', $searchid, PDO::PARAM_INT);
        $cancelSTAGES->execute()or die(print_r($cancelSTAGES->errorInfo(), true));
    
        $cancelCALLBKS = $pdo->prepare("UPDATE scheduled_pension_callbacks set complete='Y' WHERE client_id =:id");
        $cancelCALLBKS->bindParam(':id', $searchid, PDO::PARAM_INT);
        $cancelCALLBKS->execute()or die(print_r($cancelCALLBKS->errorInfo(), true));
        
        if(isset($fferror)) {
            if($fferror=='0') {
                
                header('Location: ../ViewClient.php?clientedited=cancelled&search='.$searchid); die;
...
$addquery = $pdo->prepare("UPDATE pension_clients SET residence=:residence, marital=:marital, ni_num=:ninum, ni_num2=:ninum2, title=:title, first_name=:first, initials=:initials, last_name=:last, dob=:dob, title2=:titlehold, first_name2=:firsthold, initials2=:initialshold, last_name2=:lasthold, dob2=:dobhold, address1=:add1, address2=:add2, address3=:add3, town=:town, post_code=:post, number1=:num1, number2=:num2, number3=:num3, email=:email, updated_by=:hello WHERE client_id=:clientid");
...
header('Location: ../ViewClient.php?clientedited=y&search='.$searchid); die;
    }
}
